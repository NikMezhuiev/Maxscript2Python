...
def reserved():
	...
	return regex(
		"(about|and|animate|as|at|attributes|by|case|catch|collect|continue|coordsys|do|else|exit|fn|for|from|function|global|if|in|local|macroscript|mapped|max|not|of|off|on|or|parameters|persistent|plugin|rcmenu|return|rollout|set|struct|then|throw|to|tool|try|undo|utility|when|where|while|with)(?![a-zA-Z0-9_])",
		re.IGNORECASE)
...
def with_context():
	...
	yield normalspaces()
	kw = yield keyword(
		"(animate|undo|redraw|quiet|printAllElements|defaultAction|MXSCallstackCaptureEnabled|dontRepeatMessages|macroRecorderEmitterEnabled)")
	yield normalspaces()
...
def rollout_item():
	...
	# pylint: disable=line-too-long
	kw = yield keyword(
		"dotnetcontrol|hyperlink|subrollout|multilistbox|imgtag|curvecontrol|angle|label|button|edittext|combobox|dropdownList|listbox|spinner|slider|pickbutton|radiobuttons|checkbox|checkbutton|colorPicker|mapbutton|materialbutton|progressbar|timer|bitmap|groupbox")
	yield normalspaces()
	var = yield var_name()
	yield normalspaces()
...
def rollout_handler():
	...
	yield normalspaces()
	expr = yield expression
	return s.Construct(s.ROLLOUT_HANDLER, handler_name, varn, varn2, varn3, expr)
...
def when_handler():
	...
	def when_attribute():
		...
		yield normalspaces()
		kw = yield keyword(
			"topology|geometry|names?|transform|select|parameters|subAnimStructure|controller|children|any")
		yield normalspaces()
